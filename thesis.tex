...
for an automated method of detecting and monitoring the disease. One promising method
is detecting diabetic retinopathy in retinal images.
...
different features and methods. This is done by studying the performance of Naive-Bayes, GMM-Bayes
and unsupervised edge-detection methods with varying levels of ground truth accuracy.
Results confirm the negative effect inaccurate ground truth has on segmentation accuracy.
Also method- and feature-specific performance is explored.
\end{abstract}
...
\begin{tiivis}
Diabetes on kasvava ongelma kehittyneissä maissa, joka motivoi automatisoidun taudin tunnistus- ja
seurausmenetelmän tutkimista. Yksi lupaava menetelmä on tunnistaa diabeettinen retinopatia silmänpohjakuvista.
Tämän tutkielman tavoite on tutkia epätarkan asiantuntijatiedon vaikutuksia exudaattien segmentointiin
silmänpohjakuvissa, sekä vertailla eri piirteiden ja metodien suorituskykyä. Tämä toteutetaan
vertailemalla Naive-Bayes, GMM-Bayes ja ohjaamattomien reunantunnistusmenetelmien suorituskykyä
erilaisilla asiantuntijatiedon tarkkuuksilla. Tulokset varmistavat epätarkan asiantuntijatiedon vaikutukset
...
Color was extracted from the image as-is. Edge was represented with the differential of the
...
% result.
Sensitivity represents the amount of samples correctly classified as positive. Its values range from 0 to 1, where 1
means all foreground samples were correctly classified. Specificity in turn represents the amount of background pixels
correctly described as negative. Its values also range from 0 to 1, where 1 means all background pixels were correctly classified. ~\cite{Fawcett:2006}
They are defined as:
...
The Dice coefficient~\cite{Dice:1945}, Jaccard index and F-score all measure set agreement, representing the success of segmentation in a single number.
...
As the optic disk's colour is very close to the exudates, a mask was generated with the method described in
...
Classifiers were trained with colour, edge and texture features, or a combination of the three. Feature extraction
methods are described in Sec.~\ref{subsubsec:features}. No contrast enhancement was used, so raw colour info
from the image was used for the colour feature.
...
Comparing the sensitivity and specificity of Naive-Bayes and GMM-Bayes classifiers shown in Figures~\ref{fig:bayes} and~\ref{fig:gmmbayes},
a slighty higher performance of colour-feature GMM-Bayes classifiers can be seen. The Naive-Bayes edge-classifier on the other hand,
...
linear progression of Naive-Bayes' sensitivity and specificity as the ground truth becomes more inaccurate.
...
reducing the color variance of the exudates and background. However, results with Naive-Bayes are almost identical between the image sets,
...
\begin{figure}[ht]
{
  ...
  \caption[moving]{Performance of Naive-Bayes classifiers:
...
were compared, Naive-Bayes and GMM-Bayes. Classifiers were trained with color-, edge- and texture-features.
...
impact on segmentation performance. As for the features, classifiers using the edge-feature outperformed colour- and texture-
...
GMM-Bayes was expected to outperform the simpler Naive-Bayes, but there was no clear difference in overall performance of these methods.
However, Naive-Bayes' performance with higher levels of inaccuracy in ground truth was much more predictable, as the segmentation 
...
of Naive- and GMM-Bayes, largely due to the mask used to rule out blood vessels. 

The second goal of the thesis was to evaluate the performance of features other than colour. Based on the performance of 
...
\section{Example results of Naive-Bayes}
\label{app:bayes}
\begin{figure}[ht]
{
    \label{fig:eye28_bayes_color}
    \includegraphics[width=1.5\textwidth,angle=90]{eye_bayes_color_28}     
    \caption[moving]{Example results of Naive-Bayes classifier using color}
...
\begin{figure}[ht]
{
    ...
    \caption[moving]{Example results of Naive-Bayes classifier using edge}
...
\begin{figure}[ht]
{
    ...
    \caption[moving]{Example results of Naive-Bayes classifier using texture}
...
\begin{figure}[ht]
{
    ...
    \caption[moving]{Example results of Naive-Bayes classifier using all features}
...
\begin{figure}[ht]
{
  ...
  \caption[moving]{Performance of Naive-Bayes classifiers using gray world images:
