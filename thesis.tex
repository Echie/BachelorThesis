...
The evaluation metrics considered in this thesis measure set agreement, representing the success of segmentation in a single number.
All of these coefficients can have values from the range [0,1], where 0 indicates no similarities, and 1 indicates perfect agreement.
...
In the purpose of finding best parameters for unsupervised methods, it was desired to have a single number describing 
the ''goodness'' of segmentation results to enable easier comparison and ranking of results. Different values,
...
and F-score were considered (defined in Eq.~\ref{eq:dice},~\ref{eq:jaccard},~\ref{eq:fscore}).
