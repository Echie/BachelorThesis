...
\subsection{Eye fundus features}
...
The mask is formed by first using contrast limited adaptive histogram equalization~\cite{Reza:2004} to enhance contrast in the green channel of
...
of all RGB-values in the image, and $r_{avg}$ is the mean of the specific channel. Results are shown in \textbf{KUVA}
...
this would include the exudates, but also the borders of the blood vessels and optic disc. Edge detection as such isn't
sufficient for exudate detection. These unwanted regions need to be subtracted from the initial result.
...
more loosely circled. This is illustrated in image \textbf{KUVA}.
