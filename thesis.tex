...
\textbf{KUVAKOHTAISET TULOKSET SUPERVISED METHODS, LISÄÄ APPENDIX VIITE}

\textbf{GRAY WORLD TULOKSISTA VIELÄ KUVAAJIA, tarkista gray world kappale}

\textbf{TARKISTA ETTÄ UNSUPERVISED TULOSTEN ARVIOINTI KÄYTTÄÄ TARKKAA GT}
...
when the level of inaccuracy in ground truth increases. This is also visible in the image-specific results show in \textbf{APPENDIX VIITE}.
...
a considerably higher performance of colour-feature GMM-Bayes classifiers can be seen. The Naive-Bayes edge-classifier on the other hand,
is clearly outperforming any other single-feature trained classifier or even the classifier with all features included.
...
Color normalization didn't have a big impact on the results. The expected improvement on the performance of color-based
classifiers is minimal, and the performance of edge-based classifiers was slightly worse.
...
The sensitivity of unsupervised methods behaves similarly with the supervised methods. More exudates are found
as the ground truth inaccuracy increases. However, their specificity is significantly more resistant to inaccurate
ground truth, maintaining a high value throughout the different levels of inaccurate ground truth.
...
\begin{figure}[ht]
{
  ...
  \subref{fig:unsupspec} Specificity}
  \label{fig:unsup}
...
\begin{figure}[ht]
{
    ...
    \caption[moving]{Imagewise results of Tophat}
...
%   \subfloat[]{
%      \label{fig:eye34_kirsch}
%      \includegraphics[width=1.6\textwidth,angle=90]{eye_kirsch_34}}
%      
%   \caption[moving]{Imagewise results of Kirsch}
% 
% }\end{figure}
