...
\subsection{Evaluation of unsupervised methods}
...
\begin{figure}[ht]
{
  \subfloat[]{
     \label{fig:sens}
     \includegraphics[width=\textwidth]{hist_bayes_dice_imgset1}}
  
  \subfloat[]{
     \label{fig:spec}
     \includegraphics[width=1\textwidth]{hist_gmmbayes_dice_imgset1}}
     
  \caption[moving]{Ground truths:
  \subref{fig:sens} Original accurate ground truth
  \subref{fig:spec} Inaccurate ground truth }
  \label{fig:bayes}
...
MASKATAANKO OPETUKSESSA OPTIC DISK?
TARKISTA SAMPLING (käytetäänkö koko opetussettiä)
TEORIAAN RANGE JA LBP (SUPERVISED -> FEATURES)
...
Classifiers were trained with colour, edge and texture features, or a combination of the three.
No contrast enhancement was used, so raw colour info from the image was used for the colour feature.
Range was used to represent edge, and LBP was used for texture.


Naive-bayes \& GMMBayes

preprocessing, remove timestamp, generate mask for image outline and optic disk (no vesselmask)

color (no contrast enhancement), edge (range), texture (LBP) combinations
